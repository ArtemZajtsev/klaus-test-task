...
import { Db } from "../db";
import { aggregateRatingsForScoring, getPreviousPeriod, getStartEndDate, getWeightedScore } from "../utils";
...
export async function getScoreChangeAgainstPreviousPeriod(
    ...
    try {
        ...
        const ratingsOverCurrentPeriod = await db.getRatingsWithCategoriesByPeriod(startDate, endDate);
        if (!ratingsOverCurrentPeriod.length) {
            callback({code: grpc.status.NOT_FOUND, details: 'no ratings in current period'});
        }

        const ratingsOverPreviousPeriod = await db.getRatingsWithCategoriesByPeriod(previousStartDate, previousEndDate);
        if (!ratingsOverPreviousPeriod.length) {
            callback({code: grpc.status.NOT_FOUND, details: 'no ratings in previous period'});
        }

        const currentScore = getWeightedScore(aggregateRatingsForScoring(ratingsOverCurrentPeriod));
        const previousScore = getWeightedScore(aggregateRatingsForScoring(ratingsOverPreviousPeriod));

        const scoreChangeAbsolute = currentScore - previousScore;
        const scoreChangePercentage = Math.round(((currentScore - previousScore) / previousScore) * 100);
