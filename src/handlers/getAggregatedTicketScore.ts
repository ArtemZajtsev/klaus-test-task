...
import { AggregatedTicketScore, TicketCategoryScore, TimePeriod } from "../../proto/server/klaus_pb";
import { Db } from "../db";
...
export async function getAggregatedTicketScore(call: ServerWritableStream<TimePeriod, AggregatedTicketScore>) {
        ...
        const ratingsToAggregate: RatingWithCategory[] =
         ...
         for (const [ticketId, ticketCategoriesWithRatings] of Object.entries(ratingsAggregatedByTicket)) {
            ...
            for (const [categoryId, ticketCategory] of Object.entries(ticketCategoriesWithRatings)) {
                ...
                const ticketCategoryRatingsSum = ticketCategory.ratings.reduce((acc, val) => { return acc + val}, 0);
                const ticketCategoryScoreValue = calculateAverageScore(ticketCategoryRatingsSum, ticketCategory.ratings.length);
