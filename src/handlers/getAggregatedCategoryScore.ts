...
import { AggregatedCategoryScore, PeriodScore, TimePeriod } from "../../proto/server/klaus_pb";
import { Db } from "../db";
import { RatingWithCategory, RatingsAggregatedByCategory } from "../types";
import { MONTH_IN_SECONDS, calculateAverageScore, getStartEndDate, getWeekNumber } from "../utils";
...
export async function getAggregatedCategoryScore(
    ...
    try {
        ...
        if (!ratingsToAggregate.length) {
            call.emit('error', {code: grpc.status.NOT_FOUND, details: 'no ratings in provided period'});
        }
        ...
        ratingsToAggregate.forEach((ratingWithCategory: RatingWithCategory) => {
            ...
            if (!ratingsAggregatedByCategory[currentCategoryId]) {
                ...
                ratingsAggregatedByCategory[currentCategoryId].ratingsSum += ratingWithCategory.rating;
        ...
        for (const [categoryId, aggregatedCategory] of Object.entries(ratingsAggregatedByCategory)) {
            ...
            for (const [periodName, periodRatings] of Object.entries(aggregatedCategory.ratingsByPeriod)) {
                ...
                const periodRatingsSum = periodRatings.reduce((acc, val) =>  { return acc + val }, 0);
                const periodScoreValue = calculateAverageScore(periodRatingsSum, periodRatings.length);
            ...
            aggregatedCategoryScore.setRatingcount(aggregatedCategory.ratingsCount);
            aggregatedCategoryScore.setPeriodscoreList(categoryPeriodScores);
            aggregatedCategoryScore.setScore(calculateAverageScore(aggregatedCategory.ratingsSum, aggregatedCategory.ratingsCount));
