...
export function getWeekNumber(date: Date) {
	...
	firstDayOfWeek.setUTCDate(firstDayOfWeek.getUTCDate() + 4 - (firstDayOfWeek.getUTCDay() || 7));
...
export function aggregateRatingsForScoring(ratings: RatingWithCategory[]): RatingsAggregatedForScoring {
...
export function getWeightedScore(ratingsAggregatedForScoring: RatingsAggregatedForScoring): number {
    let totalRatingsWeightedSum = 0;
    let totalWeight = 0;

    for (const [categoryId, categoryAggregated] of Object.entries(ratingsAggregatedForScoring)) {
        const categoryRatingsWeightedSum = categoryAggregated.ratings.reduce((acc, val) => {
    ...
    return Math.round((totalRatingsWeightedSum / (totalWeight * MAX_RATING)) * 100);
}
